    ...
    return (
        <div className="text-sm md:text-xl flex h-24 md:h-36 justify-between font-body font-bold text-mercury-200 hover:bg-woodsmoke-700 rounded-lg transition-colors duration-300 px-2 md:px-8 my-2">
            <div className="imagecontainer flex items-center">
                <img className="w-12 md:w-16 rounded-lg mr-2" src={`http://127.0.0.1:8000/logos/${props.data.symbol}.jpg`} alt="stock" />
            ...
            <div className="price flex flex-col justify-center items-end text-mercury-400">
                <div className="md:text-2xl text-md">{(Math.round(props.data.price * 100) / 100).toFixed(2)}</div>
                <div className={`onedaychange text-sm md:text-base text-nowrap ${props.data.positive ? "text-emerald-400" : "text-amaranth-500"}`}>
                    {props.data.positive ? "+" : "-"}
