...
const ExploreStock = (props) => {
    return (
        <div className="text-sm md:text-base flex h-16 md:h-20 justify-between font-body font-bold text-mercury-200 hover:bg-woodsmoke-700 rounded-lg transition-colors duration-200 px-2 md:px-5 my-1">
            ...
            <div className="price flex flex-col justify-center items-end text-mercury-400 tabular-nums">
                <div className="md:text-xl text-base">{(Math.round(props.data.price * 100) / 100).toFixed(2)}</div>
                <div className={`onedaychange text-xs md:text-sm text-nowrap ${props.data.positive ? "text-emerald-400" : "text-amaranth-500"}`}>
                    {props.data.positive ? "+" : "-"}
