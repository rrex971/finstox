import { useState, useEffect, useContext } from "react";
import API_BASE from "../apiConfig";
import { motion } from "framer-motion";
import { FaX } from "react-icons/fa6";
import { toast } from "react-toastify";
import GlobalContext from "../GlobalContext";

const SellDialog = ({ open, onClose, symbol, price }) => {
  const [responseOk, setResponseOk] = useState(false);
  const {navbarRefresh, setNavbarRefresh} = useContext(GlobalContext);
  const [quantityHeld, setQuantity] = useState("...");
  const [sellquantity, setSellQuantity] = useState(0);
  const [total, setTotal] = useState(0);
  const username = localStorage.getItem("username");

  useEffect(() => {
    if(open) {
      fetch(`${API_BASE}/getQtyOwned?username=${username}&symbol=${symbol}`)
        .then(response => response.json())
        .then(data => setQuantity(data.quantity));
    }
  }, [open, username, symbol])

  const updateTotal = () => {
    let total = (sellquantity * price).toFixed(2)
    setTotal(total==="NaN" ? 0 : total);
  }

  useEffect(() => {
    updateTotal();
  }, [sellquantity, price]);

  if (!open) {
    return null;
  }

  const handleSubmit = (event) => {
      event.preventDefault();
      const formData = new FormData(event.currentTarget);
      const quantity = Number(formData.get("sellquantity"));
      if (isNaN(quantity) || quantity <= 0) {
        toast.error("Please enter a valid quantity");
        return;
      }
      fetch(`${API_BASE}/sell`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          username: username,
          symbol: symbol,
          quantity: quantity
      })
      })
        .then(async response => {
          const data = await response.json();
          console.log(data);
          if(response.ok) {
            toast.success(data.detail);
          } else {
            toast.error(data.detail);
          }
          setNavbarRefresh(true);
        })
        .catch(error => {
          console.error(error);
          toast.error(error.message);
        });

      onClose();
  };

  const popupVariants = {
    hidden: {
      opacity: 0,
      scale: 0.5
    },
    visible: {
      opacity: 1,
      scale: 1,
      transition: {
        type: "spring",
        stiffness: 1000,
        damping: 100
      },
      duration: 200
    }
  };

  return (
    <motion.div
      className={`text-mercury-200 text-4xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 bg-woodsmoke-900 border-woodsmoke-800 border rounded-xl p-6 shadow-2xl w-11/12 md:w-1/2`}
      variants={popupVariants}
      initial="hidden"
      animate={open ? "visible" : "hidden"}
    >
        <form
          onSubmit={handleSubmit}
        >
          <div className="flex justify-between items-center">
            <label className="text-mercury-200 font-bold" htmlFor="quantity">Quantity:</label>
            <button
              className="text-mercury-200 rounded-lg text-lg font-semibold transition-all duration-300 hover:bg-woodsmoke-900 hover:bg-none hover:text-accent-500 hover:border-accent-500"
              type="button"
              onClick={onClose}
            >
            <FaX />
            </button>
          </div>
          <div className="my-4 text-mercury-400 text-lg font-medium">Owned: {quantityHeld}</div>
          <input
            className="w-full px-4 py-3 border border-woodsmoke-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-500"
            type="text"
            pattern="^\d+(\.\d{1,2})?$"
            inputMode="decimal"
            id="sellquantity"
            name="sellquantity"
            required
            onChange={(e) => setSellQuantity(parseFloat(e.target.value))}
          />
          {price && <div className="my-4 text-mercury-200 text-lg font-medium">Price: {price}</div>}
          {price && <div className="text-mercury-200 text-2xl font-semibold">Total: {total}</div>}
          <div className="flex justify-between items-center">
            <button
              className="w-full mt-6 py-3 text-mercury-200 bg-accent-500 hover:bg-accent-600 rounded-xl text-lg font-semibold transition-colors duration-200 cursor-pointer"
              type="submit"
            >
              Sell
            </button>
          </div>
        </form>
    </motion.div>
  );
};

export default SellDialog;

